...
function AttendanceContent() {
  ...
  useEffect(() => {
    fetch('/api/assignments', { headers: getAuthHeaders() })
      .then((r) => r.json())
      .then((data) => {
        if (data.success) {
  ...
  const loadStudentsAndAttendance = useCallback(async () => {
    ...
    try {
      ...
      const [studentsRes, attendanceRes] = await Promise.all([
        fetch(`/api/students?${params}`, { headers: getAuthHeaders() }).then((r) => r.json()),
        fetch(`/api/attendance?${params}`, { headers: getAuthHeaders() }).then((r) => r.json()),
      ])
