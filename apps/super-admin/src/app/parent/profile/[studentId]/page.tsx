...
import { getAuthHeaders, formatParentDate, studentFullName } from '@/lib/parent-portal/client-auth';
...
export default function ProfilePage() {
  ...
  const fetchTabData = useCallback(async () => {
    ...
    try {
      if (activeTab === 'overview') {
        const res = await fetch(`/api/parent/students/${studentId}/profile`, { headers });
        const data = await res.json();
        if (data.success) setProfile(data.data);
      }
      if (activeTab === 'guardians') {
        const res = await fetch(`/api/parent/students/${studentId}/guardians`, { headers });
        const data = await res.json();
        if (data.success) setGuardians(data.data);
      }
      if (activeTab === 'medical') {
        const res = await fetch(`/api/parent/students/${studentId}/medical`, { headers });
        const data = await res.json();
        if (data.success) setMedical(data.data);
      }
      if (activeTab === 'documents') {
        const res = await fetch(`/api/parent/students/${studentId}/documents`, { headers });
        const data = await res.json();
        if (data.success) setDocuments(data.data);
      }
      if (activeTab === 'history') {
        const res = await fetch(`/api/parent/students/${studentId}/enrollments`, { headers });
        const data = await res.json();
  ...
  const setTab = (tab: TabId) => {
    setActiveTab(tab);
    router.replace(parentRoute('/profile/${studentId}?tab=${tab}'));
  };
