  ...
  try {
    ...
    const threadResult = await db.query(
      `SELECT t.*,
       ...
       WHERE t.id = $1 AND t.teacher_staff_id = $2`,
      [threadId, staffId],
    );
    ...
    await db.query(
      `UPDATE parent_message_posts
       ...
       WHERE thread_id = $1 AND sender_role = 'parent' AND read_at IS NULL`,
      [threadId],
    );
    ...
    console.error('Teacher parent message thread GET:', error);
    return NextResponse.json(
...
export async function POST(
  ...
  try {
    ...
    const thread = await db.query(
      'SELECT id FROM parent_message_threads WHERE id = $1 AND teacher_staff_id = $2',
      [threadId, staffId],
    );
    ...
    const post = await db.query(
      `INSERT INTO parent_message_posts (thread_id, sender_user_id, sender_role, body)
       ...
       RETURNING *`,
      [threadId, auth.user.id, message.trim()],
    );
    ...
    console.error('Teacher parent message thread POST:', error);
    return NextResponse.json(
