...
CREATE INDEX IF NOT EXISTS idx_student_scheduled_messages_pending
  ON student_scheduled_messages (status, scheduled_at)
...
export async function ensureStudentMessagingSchema(db: RequestDb) {
  await db.query(STUDENT_MESSAGING_SQL);
}
