...
import { FEES_PORTAL_LABEL } from '@/lib/site-seo'
...
const menuItems = [
  ...
  { name: 'Fee Structures', icon: FiSettings, path: '/fee-structures' },
  { name: 'Student Fees', icon: FiUsers, path: '/student-fees' },
  { name: 'Record Payment', icon: RupeeIcon, path: '/payments' },
  { name: 'Receipts', icon: FiFileText, path: '/receipts' },
  { name: 'Reports', icon: FiTrendingUp, path: '/reports' },
  { name: 'Pending Dues', icon: FiCreditCard, path: '/pending' },
]
...
export default function Sidebar({ onToggle, mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const initialCollapsedState = useMemo(() => {
    if (typeof window !== 'undefined') {
      const savedState = localStorage.getItem('feesSidebarCollapsed')
      if (savedState === null) {
        localStorage.setItem('feesSidebarCollapsed', 'false')
        return false
      }
  ...
  const toggleSidebar = () => {
    ...
    localStorage.setItem('feesSidebarCollapsed', String(newState))
    onToggle?.(newState)
  }
      ...
      <div className={displayCollapsed ? 'p-4' : 'px-5 py-4'}>
        {displayCollapsed ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <button onClick={toggleSidebar} className="portal-sidebar-btn hidden lg:block p-2 rounded-lg transition-all duration-300" title="Expand Menu">
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs portal-sidebar-muted truncate">{FEES_PORTAL_LABEL}</p>
              </div>
            </div>
            <button onClick={toggleSidebar} className="portal-sidebar-btn hidden lg:flex flex-shrink-0 p-2 rounded-lg transition-all duration-300 mt-0.5" title="Collapse Menu">
